import requests # библиотека Python используется для отправки HTTP-запросов
import  json
from tkinter import *
...
def exchange():
    ...
    if t_code and b1_code and b2_code:
        try:
            response = requests.get(f"https://open.er-api.com/v6/latest/{b1_code}") # Для получения данных с сервера используется метод `get` библиотеки requests
...
b1_label = ttk.Label()
b1_label.pack(padx=10, pady=10) # метод `pack()` в tkinter размещает виджеты в окне
